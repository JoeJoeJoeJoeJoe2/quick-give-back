...
const Prism: React.FC<PrismProps> = ({
  ...
  useEffect(() => {
    ...
    const fragment = /* glsl */ `
      ...
      void main() {
        ...
        for (int i = 0; i < STEPS; i++) {
          ...
          p.y -= centerShift;
          d = sdPyramidUpInv(p);
          if (d < 0.001 || z > 20.0) break;
          z += d;
          o += (0.02 + 0.015 * uGlow) * abs(vec4(
            sin(cf * (f.x + f.y) * 0.05 + iTime * 0.3) + 1.0,
            cos(cf * (f.y - f.x) * 0.05 + iTime * 0.4) + 1.0,
            sin(cf * (f.x * 0.5 + f.y * 0.3) * 0.05 + iTime * 0.5) + 1.0,
            1.0
          ) / d);
        }
        ...
        if (uNoise > 0.0001) {
          float n = rand(gl_FragCoord.xy + iTime) * uNoise;
          o.rgb += n - uNoise * 0.5;
        }
    ...
    const NOISE_IS_ZERO = NOISE < 0.0001;
    let raf = 0;
    ...
    const render = (t: number) => {
      ...
      if (animationType === 'hover') {
        ...
        yaw = Math.sin(time * wY) * 0.5;
        pitch = Math.sin(time * wX + phX) * 0.3;
        roll = Math.sin(time * wZ + phZ) * 0.2;
        program.uniforms.uRot.value = setMat3FromEuler(yaw, pitch, roll, rotBuf);
      }
